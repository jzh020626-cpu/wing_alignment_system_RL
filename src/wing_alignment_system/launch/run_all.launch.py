...
from launch.actions import TimerAction, LogInfo, DeclareLaunchArgument, IncludeLaunchDescription
...
def generate_launch_description():
    ...
    workflow_arg = DeclareLaunchArgument(
        'workflow',
        default_value='full',
        description='Workflow mode: full (default), lift, or transport'
    )

    skip_preflight_arg = DeclareLaunchArgument(
        'skip_preflight',
        default_value='false',
        description='Skip workflow preflight checks for testing (use with caution)'
    )

    start_state_arg = DeclareLaunchArgument(
        'start_state',
        default_value='',
        description='Optional fine-grained mission start state override'
    )
    ...
    slide_align_mode_arg = DeclareLaunchArgument(
        ...
        description='Slide alignment mode (alignment stage is enforced to direct_only)'
    )
    ...
    start_from_loaded_arg = DeclareLaunchArgument(
        'start_from_loaded',
        default_value='false',
        description='If true, start transport workflow directly (assumes robots already aligned/loaded)'
    )
    start_from_loaded = LaunchConfiguration('start_from_loaded')

    start_trace_logger_arg = DeclareLaunchArgument(
        'start_trace_logger',
        default_value='false',
        description='If true, include wing_mechanism_bench trace logger alongside run_all.'
    )
    ...
    workflow_selected = PythonExpression([
        "'transport' if '", start_from_loaded, "'.lower() in ['true','1','yes'] else '", workflow, "'"
    ])
    ...
    ld.add_action(start_from_loaded_arg)
    ...
    ld.add_action(
        TimerAction(
            ...
            actions=[
                LogInfo(msg="===================================================="),
                LogInfo(msg="[T=12s] All robots online. Starting GLOBAL CONTROLLER..."),
                LogInfo(msg="====================================================")
            ]
        )
    )
    for action in create_global_nodes(
        ...
        sys_pkg=sys_pkg,
        config_file=config_file,
        workflow_cfg=workflow_selected,
